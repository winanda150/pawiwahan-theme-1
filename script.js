...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const initAOS = () => {
        ...
        let aosDuration = 1000; // Default Desktop: Lebih elegan dan lambat
        let aosOffset = 100;

        if (width < 768) {
            aosOffset = 50;
            aosDuration = 600; // Mobile: Ditingkatkan agar transisi terlihat jelas
        } else if (width <= 1024) {
            aosOffset = 80;
            aosDuration = 800; // Tablet: Seimbang antara responsif dan estetika
        }
    ...
    if (btnOpen && cover && music) {
        btnOpen.addEventListener('click', () => {
            ...
            // Refresh AOS lebih cepat agar sinkron dengan transisi scroll
            AOS.refresh();
    ...
    function loadGuestbook(q) {
        ...
        unsubscribeGb = onSnapshot(q, (snapshot) => {
            ...
            // Refresh AOS agar posisi elemen di bawah list ucapan (seperti Gift Section) dihitung ulang
            setTimeout(() => {
                AOS.refresh();
            }, 500);
        });
