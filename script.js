...
import { getFirestore, collection, addDoc, query, orderBy, onSnapshot, serverTimestamp, limit, doc, updateDoc, deleteDoc, getDocs, increment, deleteField, startAfter, endBefore, limitToLast, getCountFromServer } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (guestNameElement) {
        ...
        if (guestParam) {
            const decodedGuestName = decodeURIComponent(guestParam.replace(/\+/g, ' '));
            guestNameElement.textContent = decodedGuestName;

            // Isi otomatis nama di form Hybrid
            const attNameInput = document.getElementById('att-name');
            if (attNameInput) attNameInput.value = decodedGuestName.replace(/[^a-zA-Z\s]/g, '').substring(0, 20);
        } else {
    ...
    const handleOrientationChange = () => {
        if (cover && !cover.classList.contains('opened')) {
            // Reset tinggi ke 100vh agar browser menghitung ulang tinggi viewport yang baru
        ...
        btnOpen.addEventListener('click', () => {
            ...
            if (particlesContainer && particlesContainer.innerHTML === "") {
                ...
                for (let i = 0; i < particleCount; i++) {
                    ...
                    const delay = Math.random() * 2 + 's'; // Delay lebih pendek agar segera muncul
    ...
    if (statusSelect && countGroup) {
        statusSelect.addEventListener('change', (e) => {
            countGroup.style.display = e.target.value === 'Hadir' ? 'block' : 'none';
        });
    }
        ...
        hybridForm.addEventListener('submit', async (e) => {
            ...
            try {
                if (replyingToId) {
                    ...
                    // 1. Tambah balasan baru dengan adminKey untuk divalidasi Rules
                    const newReplyRef = await addDoc(repliesRef, {
                        ...
                        adminKey: isMempelai ? "mempelai123" : "", // Kirim kunci jika mode mempelai aktif
                    ...
                    // Kembalikan input kehadiran & jumlah tamu setelah berhasil membalas
                    ...
                    await addDoc(collection(db, "messages"), {
                        ...
                        count: document.getElementById('att-status').value === 'Hadir' ? document.getElementById('att-count').value : 0,
                        message: document.getElementById('att-message').value,
                        timestamp: serverTimestamp()
                    });
    ...
    document.addEventListener('click', async (e) => {
        ...
        const btnDeleteFinal = document.getElementById('btn-delete-confirm');
        const btnCancelFinal = document.getElementById('btn-cancel-confirm');
        ...
        if (likeBtn) {
            ...
            // Tambahkan animasi pop untuk feedback instan
            ...
            try {
                if (isAlreadyLiked) {
                    ...
                    likedMessages = likedMessages.filter(id => id !== docId);
                    localStorage.setItem('liked_messages', JSON.stringify(likedMessages));
                } else {
                    // PROSES LIKE
                    likeIcon.classList.replace('bi-heart', 'bi-heart-fill');

                    // Efek getar haptik (untuk smartphone) dan visual
                    if ('vibrate' in navigator) navigator.vibrate(30); 
                    likeBtn.classList.add('vibrate-effect');
                    likeBtn.addEventListener('animationend', () => {
                        likeBtn.classList.remove('vibrate-effect');
                    }, { once: true });
        ...
        if (e.target.id === 'btn-delete-confirm' && docIdToDelete) {
            try {
                if (parentIdForReply) {
                    ...
                    // Hapus pesan utama
                    await deleteDoc(doc(db, "messages", docIdToDelete));
                }
            ...
            if (replyContent) {
                ...
                if (isHidden) {
                    ...
                    querySnapshot.forEach((rDoc) => {
                        ...
                        // Konsep Nested: Tampilkan mention @nama jika membalas orang tertentu dalam utas
                        const mentionHTML = rData.replyTo && rData.replyTo !== btn.dataset.parentName
                            ? `<span class="reply-to-mention">${rData.replyTo}</span>` 
                            : '';

                        repliesHTML += `
                            <div class="gb-reply-item" style="margin-bottom: 20px;">
                                ...
                                <p class="gb-message">${mentionHTML}${rData.message}</p>
                ...
                btn.innerHTML = isHidden 
                    ...
                    : `<i class="bi bi-arrow-return-right"></i> Lihat ${count} balasan`;
            }
        }
    });
    ...
    function loadGuestbook(q) {
        ...
        unsubscribeGb = onSnapshot(q, (snapshot) => {
            ...
            if (snapshot.empty) {
                ...
                emptyItem.className = 'guestbook-item text-center';
                emptyItem.innerHTML = '<p style="color: #999; font-style: italic; margin-bottom: 0;">Belum ada ucapan. Jadilah yang pertama memberikan ucapan!</p>';
                gbList.appendChild(emptyItem);
            ...
            snapshot.forEach((doc) => {
                ...
                item.className = 'guestbook-item fade-in-up'; // Tambahkan kelas fade-in-up di sini
                item.innerHTML = `
                    <div class="gb-header-row">
                        <div class="gb-avatar">
                    ...
                    <div class="gb-actions">
                        ...
                        <button class="like-btn" data-id="${doc.id}" data-likes="${likes}">
                            <i class="bi ${isAlreadyLiked ? 'bi-heart-fill' : 'bi-heart'}"></i> <span class="like-count ${isAlreadyLiked ? 'liked' : ''}">${likes}</span>
                        </button>
                    </div>
                    ${(data.replyCount > 0) ? `
                    <div class="reply-toggle-container">
                        ...
                        <button class="view-reply-btn" data-id="${doc.id}" data-count="${data.replyCount || 0}" data-parent-name="${data.name}">
                            <i class="bi bi-arrow-return-right"></i> Lihat ${data.replyCount || 0} balasan
                        </button>
                    </div>
                    ` : ''}
                `;
                ...
                if (isLong) {
                    item.querySelector('.msg-text').dataset.full = message;
                }
