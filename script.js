...
import { getFirestore, collection, addDoc, query, orderBy, onSnapshot, serverTimestamp, limit, doc, updateDoc, deleteDoc, getDocs, increment, deleteField } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (gbList) {
        // Tampilkan loading spinner sebelum data diterima dari Firestore
        gbList.innerHTML = `
        ...
        onSnapshot(q, (snapshot) => {
            gbList.innerHTML = '';
            if (messageCounter) messageCounter.innerText = snapshot.size;
            ...
            snapshot.forEach((doc) => {
                ...
                // Format tanggal sesuai permintaan: 03 Maret 2026 pukul 14.00 Wita
                ...
                // Format tanggal balasan jika ada
                let replyDate = '';
                if (data.reply) {
                    const rDateObj = data.replyTimestamp ? data.replyTimestamp.toDate() : new Date();
                    const rDateStr = rDateObj.toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' });
                    const rTimeStr = rDateObj.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', hour12: false }).replace('.', ':');
                    replyDate = data.replyTimestamp ? `${rDateStr} pukul ${rTimeStr} Wita` : 'Baru saja';
                }
